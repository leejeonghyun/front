...
export default {
  ...
  methods: {
    sumTotalAmt() {
      if (this.ordsItemList != null && this.ordsItemList.length > 0) {
        this.totPrdAmt = 0;
        this.totPrdDcAmt = 0;
        ...
        for (let q = 0; q < this.ordsItemList.length; q++) {
          const ordsDelivGrpInfoList = this.ordsItemList[q].ordsDelivGrpInfoList;
            ...
            for (let k = 0; k < ordsDelivSupplGrpInfoList.length; k++) {
              ...
              for (let l = 0; l < ordsItemInfoList.length; l++) {
                if (ordsItemInfoList[l] !== null) {
                  // this.totPrdDcAmt += ordsItemInfoList[l].prcDcAmt + ordsItemInfoList[l].itemCpnDcAmt + ordsItemInfoList[l].dblDcCpnDcAmt;
    ...
    async retrievePntSavgRtInfo() {
      if (this.mbrGrd) {
        const iOrderRequestInputDto = {
          ordId: this.ordsItemList[0].ordId
        };

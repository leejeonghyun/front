...
<template>
  ...
  <div class="bill-wrap">
    ...
    <div  id='receiptIssueInfo' class="content-bill" v-if="showBill">
      <p class="check-area">
...
export default {
  ...
  methods: {
    ...
    async setcashRcpctInfo(e) {
      ...
      if (e !== null && e.target.checked == true) {
        ...
        if (this.cashRecptIssuTypeCd == '1' && !this.isTelNo(param.cashRecptInputNo)) {
    ...
    setUserCashRcpctInfo() { // 회원 현금영수증정보 set
      if (this.cashRcpctInfo !== undefined && this.cashRcpctInfo !== null) {
        if (this.isNotEmpty(this.cashRcpctInfo.cashRecptIssuTypeCd)) {
          this.selectBillType(null, this.cashRcpctInfo.cashRecptIssuTypeCd);
        } else {
          this.cashRecptIssuTypeCd = '1';
        }
        if (this.isNotEmpty(this.cashRcpctInfo.cashRecptInputNo)) {
    ...
    async getUserRcpInfo() {
      ...
      if (result.data.success) {
        this.cashRcpctInfo.cashRecptIssuTypeCd = result.data.data.cashRecptIssuTypeCd || '1'; // 저장된 현금영수증 정보가 없으면 Default는 개인소득공제용('1')
  ...
  async mounted() {
    await this.retrievelistCommCd();
    await this.getUserRcpInfo();
    this.cashReceiptIssueEvent();
  },
};
</script>

      ...
      <div class="sub-inner-box">
        ...
        <div ref="tabContents" tabindex="0">
          ...
          <div id="tab-contents3" class="tabcontents account" aria-labelledby="tab3" v-if="isListTab === '01'">
            ...
            <p class="text-right"><a @click="popupShinhanServiceActive()" class="btn-arrow-right" title="새 창 열림">서비스가입확인</a></p>
          </div>
          ...
          <!-- START : 현금영수증 -->
          <CashReceiptIssue v-if="this.CashReceiptIssue" @CashReceiptIssueEvent="CashReceiptIssueEvent" /> <!-- 현금영수증발급 안내 -->
          <!-- END : 현금영수증 -->
          <!-- END : 모바일팝 -->
          <!-- START : 네이버 페이 -->
...
export default {
        ...
        if (this.crcoEvntInfo != undefined && this.crcoEvntInfo != null) {
          ...
          if (this.crcoEvntInfo.tdrMeansCd === '02') {
            ...
            for (let i=0; i < crcoInfoList.length; i++) {
              if (crcoInfoList[i].pytcCd == this.crcoEvntInfo.pytcCd
  ...
  methods: {
    ...
    popupShinhanServiceActive() {
      // mdaType pc: 01, mobile: 02, App(fresh): 03, App(dalisalda): 04
      if (CookieUtils.getMdaType() == '03') {
        window.open("https://my-m.gsfresh.com/pop_ShinhanService.html");
      } else if (CookieUtils.getMdaType() == '04') {
        window.open("https://my-m.dalisalda.com/pop_ShinhanService.html");
      } else {
        window.open("/pop_ShinhanService.html", "PopShinhanService", "");
      }
    },
    ...
    async retrievelistTrdMeans() {
      ...
      if (this.dlvDelivYn !== null && 'Y' === this.dlvDelivYn) { // 업체택배가 포함되어있을경우 휴대폰 소액결제 제외한다.
        for (let i=0; i < this.tdrMeansList.length; i++) {
          if (this.tdrMeansList[i].tdrMeansCd === '10') {
            this.tdrMeansList.splice(i, 1);
          }
        }
      }

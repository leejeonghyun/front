...
export default {
  ...
  methods: {
    ...
    goGoods(itemValue) {
      // 심플리쿡 식단예약인 경우 상품 상세 진입시 체크
      if (this.$props.productResvItem) {
        if (!this.$props.productResvItemData) {
          return this.$gsdialog.alert('배송날짜를 선택해주세요.');
        }
        if (itemValue.delivTypeCd == '03') {
          const week = ['SUN', 'MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT'];
          const dayOfWeek =
            week[new Date(this.$props.productResvItemData).getDay()];
          if (dayOfWeek == 'SUN' || dayOfWeek == 'MON') {
            return this.$gsdialog.alert('식단예약이 불가능한 날짜입니다.');
          }
        }
        itemValue.productResvItemData = this.$props.productResvItemData;

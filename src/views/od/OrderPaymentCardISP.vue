    ...
    <div id="authCardIsp">
      ...
      <div class="container">
        <form id="ispForm" @submit.prevent class="form-horizontal" method="post">
          ...
          <input type="hidden" name="card_code" v-model="form.cardCode" /> <!-- 카드종류 -->
        ...
        <div id="kmotionDiv" style="display:none">
          <form class="form-horizontal" id="kmotionForm" name="kmotionForm" method="post" @submit.prevent="submit()">
            ...
            <input type="hidden" name="taxableAmt"> <!--과세대상금액 -->
            <input type="hidden" name="tax"> <!--부가세 -->
            <input type="hidden" name="taxFreeAmt" v-model="form.taxfreeAmt"> <!--면세대상금액 -->
...
export default {
  ...
  methods: {
    async init() {
      ...
      if (cardCode === "04") {
        ...
        await ApiUtils.post('/fo/pa/cardInitAuth', data).then(res => {
          ...
          that.kform.amount = this.orderSheetForm.goods_price;
          that.kform.quotaInf = this.orderSheetForm.noint_inf;
          if (deviceType === "PC") {
            ...
            that.kform.returnUrl = res.data.returnUrlMob+".hashKey_"+res.data.hashKey;
            that.kform.accpresUrl = res.data.accpresUrlMob+".hashKey_"+res.data.hashKey;
            that.kform.accpreqUrl = res.data.accpreqUrlMob+".hashKey_"+res.data.hashKey;
          }
    ...
    async doPayKmotion(deviceType, smartroCardCode) {
      ...
      console.log("doPayKmotion data : ", data);
      await ApiUtils
    ...
    getRepresentative(crcoCd) {
      ...
      if (result == "00") {
        ...
        alert("카드정보에 오류가 있습니다. 관리자에게 문의하세요.");
      }
      return result;
    },
  }

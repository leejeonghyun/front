...
<template>
  <section :class="{'section-area' : payBtnNone === false && payBtnPass === false}">
    <main>
      <!-- START : 간편, 일반결제 -->
      <PaymentNormal @popupAction="popupAction"
      ...
      <OrderAgreement @popupAction="popupAction" v-if="ordsItemList !== null && payBtnNone == false" :productItems="productItems" :strInfo="strInfo" :tab="tab" @orderAgreeChk="orderAgreeChk"/>
  ...
  methods: {
    ...
    orderPaymentProc() {
      ...
      if (validationChk) {
        ...
        this.orderSheetForm.aff_name = "GS Fresh"; // 가맹점명
      ...
      if (this.validationInfo()) {
        ...
        if (this.payType === '99' && (this.totAmt === 0 || this.payBtnNone)) { // 추가결제의 GS&POINT일경우 결제금액을 0으로해서 상위로 넘긴다. 추가결제요청건 반영
          ...
          this.popup.payEasyTotalPayFlag=true;

<!--
views/od/SC-FO-OD-GS-MP-020
-->
<template>
    <!-- START : 사은폼안내 레이어 -->
    <div id="popupSpecialGift" class="wrap-full-popup" role="dialog" aria-labelledby="popupSpecialGift" aria-modal="true" :class="[{active : this.modalActive.popupSpecialGift}, siteClass]">
    <div class="header-popup">
      <strong>금액대별 사은품 안내</strong>
    </div>
    <div class="full-popup-body scroll-area">
      <div class="inner-full-popup">
        <section class="special-gift-layer">
          <div v-for="pres in delivType.presList" v-bind:key="pres.promId">
            <div class="box-alert txt-gift-desc">
              <strong>[{{delivType.delivTypeNm}}배송]</strong>
            ...
            <div class="layer-content">
              <p class="current-price">현재 주문예정금액 : <strong>{{comma(pres.ordAmt)}}원</strong></p>
...
export default {
  ...
  methods: {
    popupAction() {
      this.$emit('popupEvent', 'popupSpecialGift');
      this.modalActiveEvent('popupSpecialGift');
    },
    comma(price) {
    ...
    getFrgfImg(frgf) {
      ...
      if (frgf.presFrgfTypeCd === '3') {
        text = '@/assets/images/img/_img_logo_coupon.jpg';
      } if (frgf.presFrgfTypeCd === '4') {
        text = '@/assets/images/img/_img_logo_gspoint.jpg';
      } else if (frgf.presFrgfTypeCd === '5') {
        text = '@/assets/images/img/_img_logo_thepop.jpg';
      } else {
        text = frgf.itemImgUrl;
      }

...
-->
<template>
  <div class="wrap-cart" :class="[{'bannerOn' : floatingBanner, 'cart-main' : (Object.entries(bsktInqRsltDto).length > 0 && bsktInqRsltDto.delivTypeDtoList.length > 0)}, siteClass]"><!-- 5월 수정 : 하단 스티키 배너 활성화 시 하단 여백 확보 클래스 위치 변경(footer 추가로 인해 변경), cart-main 클래스 추가 -->
    <SubDefaultHeader headerText="장바구니" :tabbar="false" :floatingBanner="floatingBanner"/>
    <main id="main" tabindex="0" v-if="Object.entries(bsktInqRsltDto).length > 0 && bsktInqRsltDto.delivTypeDtoList.length > 0"><!-- 접근성 : 본문 바로가기 위해서 id 값 필요, 변경 시 App.vue 파일의 본문바로가기 링크도 변경 -->
      <div class="wrapper"><!-- 4월 수정: 하단 스티키 배너 활성화 시 하단 여백 확보  -->
          ...
          <div class="cart-category-product" id="cart-category-product">
            ...
            <div v-for="(delivType, index) in bsktInqRsltDto.delivTypeDtoList" v-bind:key="index" class="wrap-category-content" :id="delivTypeId(delivType.ordTypeCd, delivType.delivTypeCd)">
              ...
              <CartProductList @popupEvent="popupAction" :bsktInqRsltDto="bsktInqRsltDto" :delivType="delivType"
                               ...
                               @openCmplItemPopup="openCmplItemPopup" @openItemOptnPopup="openItemOptnPopup"
                               :mbrDlvp="mbrDlvp"
...
<script>
  ...
  methods: {
    ...
    goLogin() {
      this.$router.push('/cu/login?returnUrl=/od/bskt');
    },
    bsktLogOut() {
    ...
    async getOrdsMbrDlvp() {
      ...
      this.mbrDlvp = ordsMbrDlvpList.find(item=>item.mbrDlvpSeqno == this.ordsMbrDlvpSeqno);
      this.getBskt("S");
    },
    goDaliDream() {

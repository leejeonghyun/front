...
<template>
    <dl>
        ...
        <dd class="thumb">
            <a href="javascript:" @click="checkboxClickAction">
                <figure>
        ...
        <dd class="price">
                <span aria-label="할인율" class="discount">
                  <strong class="num">{{dcRate | comma}}</strong>%
                </span>
            <span aria-label="할인 적용 후 가격" class="final-price">
                  <strong class="number-sale-price">{{value.dcAmt | comma}}</strong>원
                </span>
        </dd>
    </dl>
</template>
<script>
    ...
    export default {
        ...
        computed: {
            dcRate() {
                const dcRate = Math.ceil(((this.value.sellAmt - this.value.dcAmt) / this.value.sellAmt) * 100);
                return dcRate;
            },
            id() {

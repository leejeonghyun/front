<template>
  <div id="optionDefault" :class="[{'wrap-option-popup': dispType === 'MD', 'wrap-full-popup': dispType !== 'MD'}, siteClass]" role="dialog" aria-modal="true">
    ...
    <div :class="{'option-popup-body': dispType === 'MD', 'full-popup-body scroll-area': dispType !== 'MD'}">
      <div :class="{'inner-full-popup': dispType !== 'MD'}">
        ...
        <!-- START : 1. 달리드림 멤버십 가입 신청 안내 문구 -->
        <div class="option-banner" v-if="params.mallId == '11' && this.$store.state.md.isPmbs == false">
          <gs-link :to="dreamPassUrl" class="btn free-90days">달리드림 멤버십[60일 무료 신청] <span>즉시 5,000원 할인</span></gs-link>
        </div>
        <!-- END : 1. 달리드림 멤버십 가입 신청 안내 문구 -->
        ...
        <div class="option-content">
          ...
          <OptionViewList v-bind:itemSelected="itemSelected" v-bind:listData="optionList" v-bind:opv="{depth: '1', type: ''}" v-bind:params="params" v-on:selectedItem="selectedItem" v-if="optionList.length > 0 && selectedOptionActive == 0" />
          <!-- ### [E]option depth 1 ### -->
          ...
          <OptionViewList v-bind:listData="skuList" v-bind:opv="{depth: '2', type: ''}" v-bind:params="params" v-on:selectedItem="selectedItem" v-if="skuList !== null && skuList.length > 0 && selectedOptionActive == 1" />
          <!-- ### [E]option depth 2 ### -->
          ...
          <OptionViewList v-bind:listData="addCompoList" v-bind:opv="{depth: '', type: 'add'}" v-bind:params="params" v-on:selectedItem="selectedItem" v-if="addCompoList.length > 0 && selectedOptionActive == 2" />
          <!-- ### [E]추가구성 상품 ### -->

          <!-- ### [S]옵션에서 선택한 상품 ### -->
          <itemSelectedList v-bind:itemSelectedList="itemSelectedListLocal" v-bind:spv="{type: ''}" v-bind:isOptionList="isOptionList" v-bind:params="params" @deleteItem="deleteItem" @decreaseCount="decreaseCount" @increaseCount="increaseCount" @inputQty="inputQty"/>
          <!-- ### [E]옵션에서 선택한 상품 ### -->
        </div>
        ...
        <div class="option-footer">
          <!-- ##### [S] 총합계 DIV ##### -->
          <!-- ### [S]달리살다 ### -->
          <div v-if="params.mallId == '11'" class="option-total-price">
            <span class="total-title">
              <strong>총 상품금액</strong>
            </span>
            <span class="total-price">
              <span class="dalisalda-list-price">
                <span class="hidden">달리살다 멤버쉽 적용 전 가격</span>
                <span class="price-title">일반</span>{{totalBfrAmount | toCurrency}}원
              </span>
              <span class="dali-price">
                <span class="hidden">달리살다 멤버쉽 적용 후 가격</span>
                <span class="ico-badge">달리드림</span>
                <strong class="price">{{totalMbsDcAmount | toCurrency}}</strong>원
              </span>
            </span>
          </div>
          <!-- ### [E]달리살다 ### -->
          <!-- ### [S]GS Fresh 외 ### -->
          <div v-else class="option-total-price">
          ...
          <div v-if="dispType !== 'MD'" class="option-footer-btn">
            <button class="btn btn-gift" @click="setOrder('G')" v-show="giftShow">
              <span class="hidden">선물하기</span>
            </button>
...
export default {
  ...
  data() {
    return {
      siteClass: CookieUtils.getSiteClass(),
        ...
        isOptionList: false,
        dreamPassUrl: SiteUtils.dalimo(this.$store.state.md.dreamPassUrl)
    };
  },
  components: {
    itemSelectedList,
  ...
  created: async function() {
    try {
      ...
      if (Object.keys(data).length > 0) {
        this.$props.params.itemCompoCd = data.itemCompoCd;
        this.$props.params.itemCpnApplPosbYn = data.itemCpnApplPosbYn;
        this.$props.params.cardDcPosbYn = data.cardDcPosbYn;
        this.$props.params.promExcldYn = data.promExcldYn;
        if (data.itemCompoCd == '02') {
          this.$props.params.repItemId = data.itemId;
        } else {
          this.$props.params.repItemId = data.repItemId;
        }
      }
    } catch (e) {
  ...
  methods: {
    ...
    getPromInfo: async function(p) {
      try {
        if (p.itemCompoCd === '01' && p.promExcldYn === 'N') {
    ...
    decreaseCount(obj, type) {
      ...
      if (valueQty > minQty) {
        ...
        if (type == "add") {
          ...
          currentData.totQty = valueQty;
          currentData.totBfrAmt = obj.bfrSellAmt * valueQty;
          currentData.totAmt = obj.sellAmt * valueQty + (obj.skuAddAmt || 0);
          currentData.totDcAmt = obj.dcAmt * valueQty + (obj.skuAddAmt || 0);
          currentData.totMbsDcAmt = obj.mbsDcAmt * valueQty + (obj.skuAddAmt || 0);
        }
        this.updateItem();
      }
    },
    ...
    increaseCount(obj, type) {
      ...
      if (valueQty >= maxQty) {
        if (valueQty >= sellPosbQty) {
          this.$gsdialog.alert('남은 재고 수량은' + maxQty + '개 입니다');
        } else {
    ...
    inputQty(obj, type) {
      ...
      if (type == "add") {
        ...
        const currentData = this.itemSelectedListLocal[index];
        currentData.totQty = inQty;
        currentData.totBfrAmt = obj.bfrSellAmt * inQty;
        currentData.totAmt = obj.sellAmt * inQty + (obj.skuAddAmt || 0);
        currentData.totDcAmt = obj.dcAmt * inQty + (obj.skuAddAmt || 0);
        currentData.totMbsDcAmt = obj.mbsDcAmt * inQty + (obj.skuAddAmt || 0);
      }
      this.updateItem();
    },
    // 선택된 옵션 삭제
      ...
      if (itemList.length == 0) {
        ...
        if (this.isOptionList == false) {
          ...
          if (parentIndex == -1) {
            ...
            currentObject.totBfrAmt = currentObject.bfrSellAmt * currentObject.totQty;

<template>
  <div :class="[{'btn-fixed active': dispType === 'MD'}, siteClass]">
    <PopupOptionDefault v-if="this.popup.optionDefault" v-bind:itemData="itemData" v-bind:dispType="dispType" v-bind:params="params" v-bind:giftShow="giftShow" @popupAction="popupAction" @setOrder="setOrder"/>
    <PopupOptionPackage v-if="this.popup.optionPackage" v-bind:itemData="itemData" v-bind:dispType="dispType" v-bind:params="params" v-bind:giftShow="giftShow" @popupAction="popupAction" @setOrder="setOrder"/>
    ...
    <div class="wrap-btn" v-if="(itemData.ofsYn === 'N' && itemData.sellPosbQty > 0) && dispType === 'MD'">
      <button class="btn-gift lightgray" v-show="giftShow" aria-label="선물하기 / 조르기 팝업 활성화" aria-haspopup="dialog" @click="setOrder('G')"><span class="hidden">선물하기 / 조르기</span></button>
      <button class="btn-lg btn-bg darkgray" aria-haspopup="dialog" @click="setOrder('C')" data-sc-action="prod.getAtcData" data-dimension-name="prod.Detail" :data-dimension-value="itemData.itemId">장바구니</button>
...
import PopupOptionDefault from '@/views/md/OptionDefault'; // 팝업 - 일반형, 선택형 상품
import PopupOptionPackage from '@/views/md/OptionPackage'; // 팝업 - 패키지 상품
import PopupOptionResvDefault from '@/views/md/OptionResvDefault'; // 팝업 - 예약상품[일반형, 선택형}
...
import PopupToast from '@/components/common/PopupToast'; // 9월 추가 : 팝업 - 토스트 팝업
...
export default {
    ...
    return {
      siteClass: CookieUtils.getSiteClass(),
        ...
        popup: { // 팝업
          ...
          popupToastAlarm: false, // 토스트 팝업 (9월 수정 : 추가)
        },
  ...
  mounted() {
    ...
    this.getBaseDeliv(); // 기본배송정보(zipcd등) 호출
  },
  methods: {
    /* #################### Event #################### */
    popupAction(type) { // 팝업 활성/비활성
      ...
      if (type === 'dayBrkDisable' || type === 'defAddrReg') {
        if (this.popup.open == false) {
          this.closeBskt();
        }
      }
    ...
    setOrder: async function(type) {
      console.log("setOrder type IS : "+type);
    ...
    async saveBskt(type, itemSelectedList) {
      if (itemSelectedList.length > 0) {
        ...
        console.log("itemSelectedListVal :: ", itemSelectedListVal);

        let itemCompoCdVal = this.itemData.itemCompoCd;
        if (itemCompoCdVal == '03') { // 이력 : 선택형 상품 > 일반 상품 처리 > 선택형 상품 처리(2021-02-16)
          itemCompoCdVal = "03";
        }
        let resvKindVal = "10";
        if ((this.itemData.resvItemYn == 'Y' && this.itemData.resvKind == '01') || (this.deliveryDate != null && this.deliveryDate != '')) {
          resvKindVal = "50";
        }
        let grpItemId = "";
        ...
        for (let i = 0; i < itemSelectedListVal.length; i++) {
          /* if (itemSelectedListVal[i].delivTypeCd == '05') {
            if (!LoginUtils.isLogin() || this.$store.state.md.isPmbs === false) {
              this.toastMessage = "해외직구 상품은 <br/>달리드림 회원만 구매 가능합니다";
        ...
        console.log("bsktPutInputDto data:: ", bsktPutInputDto);
        ...
        try {
          ...
          if (response.data.success === true) {
            ...
            if (type === 'C') {
              ...
              if (this.dispType == "SS") {
                if (this.delivType == 'DYB' && LoginUtils.isLogin() && !await this.getDaybreakDelivRegn()) {
                  const pupCookie = CookieUtils.getCookie('BSKT_POPUP');
                  if (pupCookie != 'daybrkno') {
                    this.popup.dayBrkDisable = true;
                  } else {
                    // this.$gsdialog.alert('상품이 장바구니에 담겼습니다.');
                    this.popup.popupToastAlarm = true;
                    this.closeBskt();
                  }
                  // this.$gsdialog.alert('새벽배송이며 비로그인이거나 새벽배송불가지역.032');
                } else if (this.delivType == 'DYB' && this.baseDelivery == false) {
                ...
                if (this.$store.state.dp.daybreak && LoginUtils.isLogin() && !await this.getDaybreakDelivRegn()) {
                  const pupCookie = CookieUtils.getCookie('BSKT_POPUP');
                  if (pupCookie != 'daybrkno') {
                    this.popup.dayBrkDisable = true;
                  } else {
          ...
          this.toastMessage = "등록중 오류가 발생했습니다<br>"+e;
          this.popup.popupToastAlarm = true;
        }
        /**
        * 다음 페이지
    ...
    closeBskt() {
      let popupKey = '';
      if (this.popup.optionDefault == true) popupKey = 'optionDefault';
      else if (this.popup.optionResvDefault == true) popupKey = 'optionResvDefault';
      else if (this.popup.optionPackage == true) popupKey = 'optionPackage';
      this.popupAction(popupKey);
    },
    toBskt() {
    ...
    toGiftBskt() {
      const bsktUrl = SiteUtils.commo('/od/gift-give-cart');
      location.href = bsktUrl;
    },
    toBuy(ordId) {
      const ordUrl = SiteUtils.commo('/od/ordersheet/'+ordId);
    ...
    getOrder: async function(obj) {
      ...
      if (obj.mallId != "" && obj.mallId != undefined) {
        this.defMallId = obj.mallId;
      }
      ...
      obj.cmmSteId = this.cmmSteId;
      this.params = obj;
      this.errorMessage = "구매 할 수 없는 상품 입니다";
      try {
        // const param = {
        //   itemId: obj.itemId,
        //   mallId: obj.mallId,
        ...
        // };
        const result = await ApiUtils.get('/fo/md/iteminfomgnt/item-info', obj);
        if (result.data.data != null) {
          this.itemData = result.data.data;
          this.itemData.dspItemNm = result.data.data.itemNm;
          if (this.itemData.delivTypeCd == '') {
            // this.$gsdialog.alert('배송 정보가 없습니다.');
            // this.toastMessage = "배송 정보가 없습니다";
          ...
          if (this.itemData.supplFirmCd == '') {
            ...
            // this.toastMessage = "공급업체 정보가 없습니다";
            this.toastMessage = this.errorMessage;
            this.popup.popupToastAlarm = true;
            console.error('공급업체 정보가 없습니다.', this.itemData);
            return;
          }
          if (this.itemData.skuIdArray == null || this.itemData.skuIdArray == '') {
            // this.$gsdialog.alert('단품 정보가 없습니다.');
            // this.toastMessage = "단품 정보가 없습니다";
            this.toastMessage = this.errorMessage;
            this.popup.popupToastAlarm = true;
            console.error('단품 정보가 없습니다.', this.itemData);
            return;
          }
        } else {
          ...
          // this.toastMessage = "상품정보가 없습니다";
          this.toastMessage = this.errorMessage;
          this.popup.popupToastAlarm = true;
        }
        // 선물하기 아이콘 노출여부(제외조건 : 비로그인, 사전예약, 해외직구, 새벽배송, 마감세일, 하루판매)
        console.log("giftShow rules :: isLogin="+LoginUtils.isLogin()+" : resvItemYn="+this.itemData.resvItemYn+" : delivTypeCd="+this.itemData.delivTypeCd+" : daybreak="+this.$store.state.dp.daybreak+" : ssevntClasfCd="+this.itemData.ssevntClasfCd);
        if (LoginUtils.isLogin() && this.itemData.resvItemYn !='Y' && this.itemData.delivTypeCd != '05' && !this.$store.state.dp.daybreak && this.itemData.ssevntClasfCd != '05' && this.itemData.ssevntClasfCd != '07') {
          console.log("gift!! :: "+LoginUtils.isLogin()+" : "+this.itemData.delivTypeCd+" : "+this.itemData.resvItemYn);
          this.giftShow = true;
        }
        ...
        if (obj.mallId !== '11') {
          ...
          this.getBaseDeliv(); // 기본배송정보(zipcd등) 호출
          /*
          if (this.$store.state.dp.daybreak || this.delivType == 'DYB') {
            setTimeout(fn, 500);
        ...
        if (this.itemData.sellAgeRestrCd == '19') {
          ...
          if (!LoginUtils.isLogin()) {
            ...
            // const aad = "20190810123456".substr(0, 8);
            console.log("aad :: |"+aad+"|");
        ...
        //   console.error(e);
        // });
        this.toastMessage = "상품정보 조회중 오류가 발생했습니다";
        this.popup.popupToastAlarm = true;
        console.error(e);
      }
      ...
      if (this.$props.dispType !== 'MD') {
        // 해외배송의 경우 유료회원만 장바구니 가능
        /* if (this.itemData.delivTypeCd == '05') {
          if (!LoginUtils.isLogin() || this.$store.state.md.isPmbs === false) {
            this.toastMessage = "해외직구 상품은 <br/>달리드림 회원만 구매 가능합니다";
            this.popup.popupToastAlarm = true;
            return;
        ...
        this.$store.commit('delItemSelectedList'); // 유닛으로 접근했을때 기존 선택옵션삭제
        /*
        [itemCompoCd][01:일반상품, 02:패키지, 03:선택형, 04:대용량정육]
        [resvItemYn][Y:예약상품]
        [resvKind][01:날짜 지정형, 02:날짜 고정형]
        ...
        if (this.itemData.itemCompoCd == '01' || this.itemData.itemCompoCd == '03') {
          if (this.itemData.ofsYn == 'Y' || this.itemData.sellPosbQty == 0) {
            if (this.popup.rstkinInfm == false) {
              console.log("getRstkinInfmRqt call");
            ...
            if (this.itemData.itemCompoCd == '01') {
              let showCart = false;
              const url = "/fo/md/iteminfomgnt/normal-item-list";
              try {
                const result = await ApiUtils.get(url, obj);
                if (result.data.data != null) {
                  if (result.data.data.length == 1) {
                    const result2 = await ApiUtils.get('/fo/md/iteminfomgnt/add-component-item-list', obj);
                    if (result2.data.data != null && result2.data.data.length > 0) {
                      showCart = true;
                    } else {
                      const result3 = await ApiUtils.get('/fo/md/iteminfomgnt/prom-info', obj);
                  ...
                  } else { // normal-item-list 가 1개 존재하고 추가구성상품과 프로모션사은품이 존재하지 않는 경우에만 장바구니 바로담기
                    const obj = result.data.data[0];
                    obj.itemNm = this.itemData.itemNm;
                    obj.dispItemNm = this.itemData.itemNm;
                    if (this.dispType == "SS") {
                      if (this.delivType == 'DYB') {
                        obj.delivTypeCd = '04';
                      } else {
                        obj.delivTypeCd = this.itemData.delivTypeCd;
                      }
                    } else {
                  ...
                  this.toastMessage = "상품정보가 존재하지 않습니다";
                  this.popup.popupToastAlarm = true;
                }
              } catch (e) {
    ...
    getRstkinInfmRqt: async function() {
      ...
      try {
        if (!LoginUtils.isLogin()) {
          ...
          this.toastMessage = "로그인 후에 이용 가능합니다";
          this.popup.popupToastAlarm = true;
        } else {
          console.log("getRstkinInfmRqt Login case start");
          ...
          if (request.data.success === true ) {
            if (request.data.data.length > 0) {
              ...
              this.toastMessage = "이미 재입고 알림을 신청 하셨습니다";
              this.popup.popupToastAlarm = true;
            } else {
              this.popup.open = true;
              this.popup.rstkinInfm = true;
        ...
        // });
        this.popup.open = true;
        this.toastMessage = "재입고알림 신청중 오류가 발생했습니다";
        this.popup.popupToastAlarm = true;
        console.error(e);
      }
    },
          ...
          if (request.data.success === true ) {
            if (request.data.data.length == 0) {
              ...
              try {
                ...
                this.params.telNo = cuInfo.moblTelNo;
              } catch (e) {
                console.error("회원모바일번호조회에러 : "+e);
              }
              const skuIdArr = this.itemData.skuIdArray.replace("{", "").replace("}", "").split(",");
              if (skuIdArr.length > 0) {
                this.params.skuId = skuIdArr[0].replace(/"/gi, '');
              }
              const response = await ApiUtils.post('/fo/md/iteminfomgnt/restock-inform', this.params);
              if (response.data.success === true) {
                this.popup.rstkinInfm = false;
                // this.$gsdialog.alert('재입고 알림이 신청 되었습니다.<br/>입고 시 안내 드리겠습니다.', {html: true});
                this.toastMessage = "재입고 알림이 신청 되었습니다.<br/>입고 시 안내 드리겠습니다";
                this.popup.popupToastAlarm = true;
              } else {
                // this.$gsdialog.alert('재입고알림 신청중 오류가 발생했습니다.').then(dialog => {
                //   console.error(e);
    ...
    getDaybreakDelivRegn: async function() {
      ...
      try {
        ...
        if (result.data.data != null) {
          ...
          return (daybreakDelivRegn.length > 0);
        } else {
          return false;
        }
      } catch (e) {

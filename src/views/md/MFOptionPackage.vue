<template>
  <div id="optionPackage" :class="{'wrap-option-popup': dispType === 'MD', 'wrap-full-popup': dispType !== 'MD'}" role="dialog" aria-modal="true">
    <div v-if="dispType !== 'MD'" class="header-popup">
      <strong>옵션 선택</strong>
    </div>
    <div :class="{'option-popup-body': dispType === 'MD', 'full-popup-body scroll-area': dispType !== 'MD'}">
      <div v-if="dispType === 'MD' && isOptionList == true" class="item-title">
        <strong>{{item.itemNm}}</strong>
      </div>

      <div class="option-content">
        <template v-if="isOptionList == true">
        ...
        <!-- [S]옵션에서 선택한 리스트 -->
        <itemSelectedList :itemSelectedList="itemSelectedListLocal" :spv="{type: 'pkg'}" v-bind:isOptionList="isOptionList" @deleteItem="deleteItem" @decreaseCount="decreaseCount" @increaseCount="increaseCount" @inputQty="inputQty"/>
        <!-- [E]옵션에서 선택한 리스트 -->
      </div>
    ...
    getOptItemList: async function() {
      try {
        ...
        if (result.data.data != null) {
          ...
          // 선택할 단품이 없을경우...
          if (this.optionList.length == this.nonSkuIdx.length) {
            this.optionList.forEach(function(data, idx) {
              vm.selectedItem(data, '1', 'NONSKU');
            });
      ...
      if ((this.optionList.length - this.nonSkuIdx.length) == objCnt) {
        ...
        this.nonSkuIdx.forEach(function(data, idx) {
          const setObj = {
            ...
            delivTypeCd: CookieUtils.isDayBreak() ? '04' : vm.optionList[data].delivTypeCd
          };
        ...
        this.tempItemSelectList.forEach(function(data, idx) {
          ...
          setObj.delivTypeCd = CookieUtils.isDayBreak() ? '04' : data.delivTypeCd;
        });

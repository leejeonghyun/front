  ...
  <transition name="popupOther" v-if="popup" v-on:enter="popupOtherEnter" v-on:after-enter="popupOtherAfterEnter" v-on:leave="popupOtherLeave">
    <div id="intergratedUI" class="wrap-intergrated-popup" role="dialog" tabindex="0" aria-modal="true" :class="[{active : popupActive}, channelPosition]">
      ...
      <div class="layer-inner">
        <div class="text" v-if="Object.keys(repBnrLevelOne).length > 0">
          <vue-typed-js :class="'text-info'" :strings="[repBnrLevelOne.repBnrImgDesc]" :typeSpeed="50" :startDelay="500" :showCursor="false">
            <span class="typing"></span>
          </vue-typed-js>
          <div class="logo">
            <button class="btn-marketfor-popup" aria-haspopup="dialog" @click="goMarketForHome">
              <img :src="repBnrLevelOne.repBnrImgPathNm" :alt="repBnrLevelOne.repBnrImgDesc" />
        ...
        <div class="text" v-else>
          ...
          <div class="logo">
            <button class="btn-marketfor-popup" aria-haspopup="dialog" @click="goMarketForHome">
              <img src="@/assets/images/_temp/_logo_gsfresh.png" alt="GSfresh mall" />
              <span class="hidden">마켓포 소개 팝업 열기</span>
            </button>
          </div>
        </div>
        ...
        <div class="wrap-circle-mall" v-if="bnrLevelOne != null">
          <ul ref="circleMall" class="circle-mall">
            <li v-for="(data, index) in bnrLevelOne" :key="index">
              <gs-link :to="data.bnrImgLinkUrl" :class="mallActiveClass(data.mallId)">
...
export default {
  ...
  data() {
    return {
      ...
      mallRotateView: 3,
      // mall active index 값
      mallActive: CookieUtils.getCookie('MALL_ID'),
  ...
  computed: {
    mallActiveClass(p) {
      return (p) => {
        return p === this.mallActive ? 'active' : '';
      };
    }
  },

...
import IspAuthOd from '@/views/pa/common/IspAuthOd';
  ...
  data() {
    return {
      cardCodeList: [
        ...
        { code: '04', name: '국민카드', typeCd: 'KMOTION' },
  ...
  methods: {
    ...
    async getAuthentication() {
      ...
      if (cardCode == '41' || cardCode == '20' || cardCode == '71' ||
        ...
        document.getElementById('KVP_CARDCOMPANY').value = formData.KVP_CARDCOMPANY;
        document.getElementById('VP_BC_ISSUERCODE').value = formData.VP_BC_ISSUERCODE;
        document.getElementById('KVP_PGID').value = formData.KVP_PGID;
        document.getElementById('KVP_CURRENCY').value = formData.KVP_CURRENCY;
        document.getElementById('KVP_QUOTA_INF').value = formData.KVP_QUOTA_INF;
        document.getElementById('KVP_NOINT_INF').value = formData.KVP_NOINT_INF;
        ...
        if (formData.deviceType === "PC") {
          ...
          const data = {
            ...
            cardQuota: ((formData.KVP_QUOTA_INF.length === 1 && formData.KVP_QUOTA_INF !== "1") ? "0" + formData.KVP_QUOTA_INF : formData.KVP_QUOTA_INF) // 할부개월수
          };
          const that = this;
          let isFail = false;
          await ApiUtils.post('/fo/pa/mobileIspReqAuthTradeKeyByLogin', data)
            .then(function(res) {
              ...
              if (res.data === null || res.data.resultCode !== '0000') {
                ...
                if (res.data.appYn === "Y") {
                  ...
                  callApp($('#MPI_FRAME'), isp); // common.app.js 필요
                }
              }
          });
          if (!isFail) {
            setTimeout(()=> { // 3초 후 부터 callback 확인
              ...
              const timer = setInterval(() => {
                console.log(count);
                ApiUtils
                  .post('/fo/pa/retrieveMobileIspAuthStateByLogin', data)
                  .then(res => {
                    if (res.data === null || (res.data.resultCode !== '0000' && res.data.resultCode !== '7896' && res.data.resultCode !== '9999')) {
                      ...
                      if (res.data !== null) {
                        if (res.data.resultCode === '0000') {
                          setTimeout(()=> { // 1초 후에 프로세스가 동작하도록 수정
                            authFinish(isFail);
                          }, 1000);
                        } else {
                          alert(res.data.resultMsg);

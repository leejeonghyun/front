  ...
  <div class="wrap-mypage member" v-bind:class="[siteClass]">
    ...
    <!-- 수정 END : 2020.02.13 bg-gray 클래스 삭제 -->
      ...
      <section class="section-area">
        ...
        <div class="box-point">
          <h3 class="title gspoint">GS&POINT</h3>
          <div class="wrap-txt-point">
            <p class="txt-point">
              <span class="num point">{{gsTotalPoint}} P
            </span>
            <p class="txt-point">
              <span class="label">익월 소멸예정</span>
              <span class="num">{{gsExtinctionPoint}} P</span>
            </p>
          </div>
        </div>
      </section>
      <section class="section-area">
        <h2 class="hidden">정회원 GS포인트 내역</h2>
        <div class="wrap-point">
          <!-- START : tab -->
          <!-- 4월수정 START : UI 변경 -->
          <div class="period-details">
            <fieldset>
              <legend>GS포인트 기간조회</legend>
              <div class="term">
        ...
        <div class="wrap-tab">
          ...
          <div ref="tabContents">
            <div id="tab1-contents" class="tabcontents" aria-labelledby="tab1" v-if="this.tab.tab1">
              <!-- START : 내역이 없을 경우 -->
              <NoList :ListType="'reviewList'" SearchText="" NoListText="포인트 내역이 없습니다." v-if="gsPointListCnt == 0"/>
              <!-- END : 내역이 없을 경우-->
              <ul class="list-point">
                <li
                v-for="gsPoint in gsPointList"
                v-bind:key="gsPoint.num"
                v-bind:gsPoint="gsPoint">
                  <div class="row">
                    <p class="date">{{ convertFormat(gsPoint.apprvDate) }}</p>
                    <p class="name"> {{ gsPoint.subCoFrnchseCodeName }}</p>
                  </div>
                  <div class="num" v-if="gsPoint.rsvPt != '0'">+ {{ toCommaCvert(gsPoint.rsvPt) }} P</div>
                  <div class="num minus" v-else>{{ toCommaCvert(gsPoint.usePt) }} P</div>
                </li>
              </ul>
            <div id="panel"></div>
            </div>
            <div id="tab2-contents" class="tabcontents" aria-labelledby="tab2" v-if="this.tab.tab2">
               <!-- START : 내역이 없을 경우 -->
              <NoList :ListType="'reviewList'" SearchText="" NoListText="포인트 내역이 없습니다." v-if="gsPointSaveListCnt == 0"/>
              <!-- END : 내역이 없을 경우-->
              <ul class="list-point">
                <li
                v-for="gsSavePoint in gsPointSaveList"
                v-bind:key="gsSavePoint.num"
                v-bind:gsSavePoint="gsSavePoint">
                  <div class="row">
                    <p class="date">{{ convertFormat(gsSavePoint.apprvDate) }}</p>
                    <p class="name"> {{ gsSavePoint.subCoFrnchseCodeName }}</p>
                  </div>
                  <div class="num">+ {{ toCommaCvert(gsSavePoint.rsvPt) }} P</div>
                </li>
              </ul>
            </div>
            <div id="tab3-contents" class="tabcontents" aria-labelledby="tab3" v-if="this.tab.tab3">
              <!-- START : 내역이 없을 경우 -->
              <NoList :ListType="'reviewList'" SearchText="" NoListText="포인트 내역이 없습니다." v-if="gsPointUseListCnt  == 0"/>
              <!-- END : 내역이 없을 경우-->
              <ul class="list-point">
                <li
                v-for="gsUsePoint in gsPointUseList"
                v-bind:key="gsUsePoint.num"
                v-bind:gsUsePoint="gsUsePoint">
                  <div class="row">
                    <p class="date">{{ convertFormat(gsUsePoint.apprvDate) }}</p>
                    <p class="name"> {{ gsUsePoint.subCoFrnchseCodeName }}</p>
                  </div>
                  <div class="num minus" >{{ toCommaCvert(gsUsePoint.usePt) }} P</div>
                </li>
              </ul>
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      /* 4월수정 END : 데이터 추가 */
      gsPointList: [],
      gsPointUseList: [], // 사용포인트
      gsPointSaveList: [], // 적립포인트
      gsPointDate: -15,
      gsTotalPoint: 0,
      gsExtinctionPoint: 0,
      gsBetweenDiv: 'day',
      gsPointListCnt: 0, // 전체내역 리스트 수
      gsPointUseListCnt: 0, // 사용포인트 리스트 수
      gsPointSaveListCnt: 0, // 적립포인트 리스트 수
      // 더보기
      moreData: true,
      loading: false,
      count: 0,
  ...
  computed: {
    // 사용안함
      totalGsPoint() {
        let total = 0;
        for (let idx = 0; idx < this.gsPointList.length; idx++) {
          total += Number(this.gsPointList[idx].usePt) + Number(this.gsPointList[idx].rsvPt);
        }
        return total;
      }
    },
 ...
 /* 4월수정 START : 이벤트 추가 */
    activeMonth(idx) { // 기간 조회 버튼 활성화 변경
      ...
      if (idx == 0) {
        ...
        this.period.start.date = DateUtils.format(DateUtils.add(DateUtils.getToday(), -90, 'day'), 'YYYY-MM-DD'); // = DateUtils.format(DateUtils.add(DateUtils.getToday(), -3, 'month'), 'YYYY-MM-DD');
        this.period.end.date = DateUtils.format(DateUtils.getToday(), 'YYYY-MM-DD');
      }
    },
    ...
    getTabAction(type) {
      ...
      this.pageSize = this.PAGE_SIZE * 2;
       // this.handleScroll();
       this.getGsPointList(DateUtils.format(this.period.start.date, 'YYYYMMDD'),
    ...
    search() {
      ...
      */
      if (DateUtils.format(this.period.start.date, 'YYYYMMDD') < DateUtils.format(DateUtils.add(this.period.end.date, -90, 'day'), 'YYYYMMDD')) {
        this.period.start.date = DateUtils.format(DateUtils.add(this.period.end.date, -90, 'day'), 'YYYY-MM-DD');
        this.$gsdialog.alert("기간조회는 최대 90일 조회 가능합니다.");
        return;
      }
    //  this.tabAction('tab1');
      this.pageNo = 1;
    ...
    getGsPointList: async function(startDt, endDt, pgNo, pgSize) {
     // console.log(" pgSize " + pgSize);
      const GsPntUseSateInqInputDto = {
            fromDate: startDt, // '20180101', // between, //
            toDate: endDt, // DateUtils.format(DateUtils.getToday(), 'YYYYMMDD'),
            pageNo: 1,
            pageSize: pgSize,
      };
      console.log(JSON.stringify(GsPntUseSateInqInputDto));
      const resultDetail = await ApiUtils.get('/fo/cu/mbrmgnt/gs-point-use-statement', GsPntUseSateInqInputDto);
      this.gsPointList = resultDetail.data.data.gsPntUseSateDtoList;
      console.log(JSON.stringify(resultDetail.data.data.totCount));
       console.log(JSON.stringify(this.gsPointList));
      this.gsPointUseList = [];
      this.gsPointSaveList = [];

      if (!StringUtils.isEmpty(this.gsPointList)) {
      this.gsPointListCnt = this.gsPointList.length;
      this.gsPointUseListCnt = 0;
      this.gsPointSaveListCnt = 0;

      let j = 0;
        for (let i = 0; i < this.gsPointList.length; i++) {
          if (this.gsPointList[i].rsvPt == '0') {
            this.gsPointUseList[j] = this.gsPointList[i];
            j = j + 1;
          }
        }
        this.gsPointUseListCnt = j;
        j = 0;
        for (let i = 0; i < this.gsPointList.length; i++) {
          if (this.gsPointList[i].usePt == '0') {
            this.gsPointSaveList[j] = this.gsPointList[i];
            j = j + 1;
          }
        }
        this.gsPointSaveListCnt = j;
      } else {
        this.gsPointListCnt = 0;
        this.gsPointUseListCnt = 0;
        this.gsPointSaveListCnt = 0;
      }

      this.totCount = Number(resultDetail.data.data.totCount);
     // this.pageSizeGap = Number(resultDetail.data.data.totCount) - this.pageSize;
    },
    getTotalGsPointList: async function() {
      const GsPntInqInputDto = {
    };
    const result= await ApiUtils.get('/fo/cu/mbrmgnt/gs-point', GsPntInqInputDto);
    this.gsTotalPoint = this.toCommaCvert(Number(result.data.data.totRestPt)); // 잔여 포인트
    this.gsExtinctionPoint = this.toCommaCvert(Number(result.data.data.nextmmVanishIntendPt)); // 소멸 포인트
    },
    getToggleAction(type, div, between) {
      this.gsPointDate = between;
      this.gsBetweenDiv = div;
    ...
    //  }
      ...
      await this.getGsPointList(DateUtils.format(this.period.start.date, 'YYYYMMDD'),
                                DateUtils.format(this.period.end.date, 'YYYYMMDD'),
                                1,
                                this.pageSize);
      this.loading = false;
  ...
  mounted: function() {
    ...
    this.period.end.date = DateUtils.format(DateUtils.getToday(), 'YYYY-MM-DD');
   // this.handleScroll();
   // this.getGsPointList(DateUtils.format(DateUtils.add(DateUtils.getToday(), this.gsPointDate, 'day'), 'YYYYMMDD'), 1, this.pageSize);
   this.getGsPointList(DateUtils.format(this.period.start.date, 'YYYYMMDD'),
                      DateUtils.format(this.period.end.date, 'YYYYMMDD'),
                      1,
                      this.pageSize);
    this.getTotalGsPointList();
    // 날짜, 전체 초기화
    this.toggleAction('toggle1');
    this.tabAction('tab1');
    // console.log(JSON.stringify(resultDetail.data.data.gsPntUseSateDtoList[0].apprvDate));

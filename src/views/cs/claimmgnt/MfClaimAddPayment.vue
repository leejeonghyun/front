<template>
  ...
  <section class="mypage block">
    <div class="total-payment order-payment mt0">
      <div class="sub-title border" v-show="addPaymentYn">
        <h3>
...
<script>
import MfPaySheet from '@/views/od/MfPaySheet';
...
export default {
  ...
  data() {
    return {
      ...
      productNm: this.$props.addPaymentYn
        ? ClaimMsg.productNmPay
        : ClaimMsg.productNmPayByUrl,
      cashRcpctInfo: {
  ...
  methods: {
    ...
    async orderProcess(rqForm, rsForm) {
      ...
      if (payType == '02') {
        // 신용카드
        iOrderRequest.mid = this.isNotEmpty(this.pytcTrmnId)
          ? this.pytcTrmnId
          : rsForm.mid; // mid (치환)
        ...
        iOrderRequest.payCompany = this.isNotEmpty(this.pytcCd)
          ? this.pytcCd
          : rqForm.payCompanyCd; // 결제사코드 (치환)
        iOrderRequest.payCompanyNm = this.isNotEmpty(this.pytcNm)
          ? this.pytcNm
          : rqForm.payCompany; // 결제사명 (치환)
    ...
    async __validation() {
      ...
      if (
        ...
        if (cashRecptInputNo.length < cnt) {
          ...
          return this.$gsdialog.alert(
            OrderMsg.cashRecptChk(this.cashRcpctInfo.cashRecptIssuTypeCd)
          );
        } else {
          this.validationChk = true;
        }
    ...
    async retrieveOrgPaymentInfo() {
      ...
      await ApiUtils.get('/fo/cs/claimmgnt/claim-org-payment-info', params)
        .then(res => {
    ...
    // GS&POINT 조회
    getGsNPoint() {
      ApiUtils.get('/fo/cu/mbrmgnt/gs-point')
        .then(res => {

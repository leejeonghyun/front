<template>
  <!-- : 실사용 소스 -->
  <div
    id="popupTrackingShipping"
    class="wrap-full-popup"
    role="dialog"
    aria-modal="true"
    :class="[{active : this.modalActive.popupTrackingShipping}, siteClass]"
  >
...
export default {
  ...
  methods: {
    popupAction() {
      ...
      if ($stickytab && window.pageYOffset >= 60) {
  ...
  mounted() {
    ...
    const appMode = Config.server.VUE_APP_MODE;
    baseDomain +=
      (appMode == 'production' ? 'GSTDS' : 'GSTDSTEST') +
      '/' +
      this.mbrDelivParamInfo.dlvcoCd +
      '/' +
      this.mbrDelivParamInfo.dlvInvNo;
    // baseDomain += '/' + 'cd2' + '/' + 'no2';

...
const UseNotWords = {
  getProhibitWords: function (useWords) {
    let rtnWords = '';
    const prhbtWords = this.getWords();
    const wordList = useWords.replace(/[\s\{\}\[\]\/?.,;:|\)*~`!^\-+<>@\#$%&\\\=\(\'\"]/gi, "@@").split('@@');
    for (let w = 0; w < wordList.length; w++) {
      for (let n = 0; n < prhbtWords.length; n++) {
        if (wordList[w] == prhbtWords[n]) {
          rtnWords += (rtnWords == '' ? '' : ', ') + prhbtWords[n];
          break;
        }
      }
    }
